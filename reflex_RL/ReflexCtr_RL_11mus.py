...
class MyoLocoCtrl_RL(object):
    ...
    cp_keys = [
        ...
        'Tr_St_sup', 'Tr_Sw', #2
        'knee_tgt', 'knee_sw_tgt', 'knee_off_st', 'ankle_tgt', #4
        '1_GLU_FG', '1_VAS_FG', '1_SOL_FG', #3
    ...
    # Calculated from the absolute max different of params at 0.8 and 1.6 against 1.2
    mod_keys_gains = np.array([
        10*np.pi/180, 6*np.pi/180, 0.5*np.pi/180, 0.7*np.pi/180, 9*np.pi/180,
        6*np.pi/180, 4*np.pi/180, 3*np.pi/180, 8*np.pi/180, 
        0.7, 0.04,
        0.02, 0.4, 0.7,
        0.04, 0.27, 0.45, 0.45,
        0.2, 0.2, 0.15, 0.06, 0.25,
    ...
    # Target gains for S < 0.14
    mod_keys_Target = [
        'theta_tgt', 'alpha_0', 'C_d', 'C_v',
        'knee_tgt', 'ankle_tgt',
        'Tr_St_sup',
        '1_SOL_FG',
        '2_BFSH_PG','2_GAS_FG', #4
        '3_HFL_Th', '3_HFL_d_Th', '3_GLU_Th', '3_GLU_d_Th',
        '4_HFL_C_HAM_PG', '4_GLU_C_RF_PG', 
        '6_HFL_RF_PG', '6_HFL_RF_VG', '6_GLU_HAM_PG',
        '8_RF_VG',
        '9_HAM_PG', '9_BFSH_HAM_Thr', '9_GAS_HAM_SG',
        '10_VAS_PG',
    ]
    mod_keys_Target_clip = [
        'Tr_St_sup',
        '1_SOL_FG',
        '2_BFSH_PG','2_GAS_FG', #4
        '3_HFL_Th', '3_HFL_d_Th', '3_GLU_Th', '3_GLU_d_Th',
    ...
    # Use the base value from the
    mod_keys_Target_gains = np.array([
        10*np.pi/180, 7*np.pi/180, 2*np.pi/180, 10*np.pi/180,
        6*np.pi/180, 8*np.pi/180,
        0.8,
        0.75,
        0.45, 0.5,
        0.2, 0.1, 0.15, 0.06, 
    ...
    # Target gains for Simplified (Wang)
    mod_keys_Simple = [
        'theta_tgt', 'alpha_0', 'alpha_delta', 'C_d', 'C_v',
        'knee_tgt', 'knee_sw_tgt', 'knee_off_st', 'ankle_tgt',
        'Tr_St_sup', 'Tr_Sw',
        '1_SOL_FG',
        '2_GAS_FG',
        '3_HFL_Th', '3_HFL_d_Th', '3_GLU_Th', '3_GLU_d_Th',
        '9_BFSH_HAM_Thr', '9_GAS_HAM_Thr', #5
    ]
    mod_keys_Simple_clip = [
        'Tr_St_sup', 'Tr_Sw',
        '1_SOL_FG',
        '2_GAS_FG',
        '3_HFL_Th', '3_HFL_d_Th', '3_GLU_Th', '3_GLU_d_Th',
        '9_BFSH_HAM_Thr', '9_GAS_HAM_Thr',
    ]
    # Use the base value from the
    mod_keys_Simple_gains = np.array([
        10*np.pi/180, 6*np.pi/180, 0.5*np.pi/180, 0.7*np.pi/180, 9*np.pi/180,
        6*np.pi/180, 4*np.pi/180, 3*np.pi/180, 8*np.pi/180, 
        0.7, 0.04,
        0.75,
    ...
    #PHI2LOPT_BFSH = 0.1988 # from matlab, when angle change is 1 rad/s, provide 0.32 simulation to muscle

    PHI_0_GLU = np.deg2rad(182)
    #PHI2LOPT_GLU = 0.3625 # From matlab, when angle change is 1 rad/s, provide 0.32 simulation to muscle

    PHI_0_HFL = np.deg2rad(175)
    #PHI2LOPT_HLF = 0.3625 # From matlab, when angle change is 1 rad/s, provide 0.32 simulation to muscle

    ALP_0_RF = (np.deg2rad(170)-np.deg2rad(125)/2)
    #A2LOPT_RF = 0.24 # From matlab, when angle change is 1 rad/s, provide 0.32 simulation to muscle

    ALP_0_HAM = (np.deg2rad(155)-np.deg2rad(180)/2)
    #A2LOPT_HAM = 0.3271 # From matlab, when angle change is 1 rad/s, provide 0.32 simulation to muscle
    ...
    def __init__(self, params=None, control_dimension=2, timestep=0.01, debug_mode=False, delayed=False):
        ...
        self.cp = {}
        self.memCP = {} # Memory to store a copy of the initial CPs to "reset" the reflex controller
        self.modCP = {} # Memory to store the modulated control params. Updated at every update step
        
        self.control_dimension = control_dimension

        if self.control_dimension == 3:
            self.n_par = len(MyoLocoCtrl_RL.cp_keys)
            params = np.ones(len(MyoLocoCtrl_RL.cp_keys))
        ...
        self.set_control_params(params) #self.reset(params) # Default set of params, if none are given
    ...
    def reset(self, params=None):
        ...
        self.cp = copy.deepcopy(self.memCP)
        self.modCP = copy.deepcopy(self.memCP)

    def reset_spinal_phases(self, init_pose):
        """
        Assumes initial pose named 'walk' in XML starts with the left leg on the ground
        """
        ...
        # walk is the default above, with left leg on ground
    ...
    def create_delay_struct(self):
        ...
        if self.control_dimension == 3:
            ...
            self.sens_leg_delay['module_phi_hip_rot'] = ShortDelay
            self.sens_leg_delay['module_dphi_hip_rot'] = ShortDelay
        ...
        self.out_leg_delay['TOFL'] = LongDelay
        self.out_leg_delay['TOEX'] = LongDelay
        self.out_leg_delay['HIPINTR'] = ShortDelay
        self.out_leg_delay['HIPEXTR'] = ShortDelay
    ...
    def reset_delay_buffers(self, initial_values, init_pose, initial_musc_ctrl, updateFlag=False):
        ...
        for leg in ['r_leg', 'l_leg']:
            ...
            for delay_item in self.out_leg_delay.keys():
                ...
                if delay_item in ['GLU', 'HAM', 'HFL', 'RF', 'HAD', 'HAB', 'BFSH', 'VAS', 'SOL', 'GAS', 'TA', 'TOFL', 'TOEX', 'HIPINTR', 'HIPEXTR']:

                    if delay_item in initial_musc_ctrl[f"{self.control_dimension}D"][leg].keys():
                        temp_ctrl = initial_musc_ctrl[f"{self.control_dimension}D"][leg][delay_item]
                    else:
                        temp_ctrl = 0.01
        ...
        # print(f"Init: {initial_values}")
        # Generate outputs for the initial state based on reflex controller if the flag is set
        if updateFlag:
            ...
            for leg in ['r_leg', 'l_leg']:
                for delay_item in self.out_leg_delay.keys():
                    temp_queue = deque(maxlen=int(np.round(self.out_leg_delay[delay_item]/self.timestep_size)))
                    [temp_queue.append(self.output_delay_data[leg][delay_item][-1]) for i in range( int(np.round(self.out_leg_delay[delay_item]/self.timestep_size)) )]
                    self.output_delay_data[leg][delay_item] = copy.deepcopy(temp_queue)

                for delay_item in self.out_c_aff_delay.keys():
                    temp_queue = deque(maxlen=int(np.round(self.out_c_aff_delay[delay_item]/self.timestep_size)))
                    [temp_queue.append(self.output_c_aff_delay_data[leg][delay_item][-1]) for i in range( int(np.round(self.out_c_aff_delay[delay_item]/self.timestep_size)) ) ]
                    self.output_c_aff_delay_data[leg][delay_item] = copy.deepcopy(temp_queue)
    ...
    def spinal_control_leg(self, s_leg, c_afferent, c_spinal):
        ...
        # Does not need further checks, since GLU_3 is already constrained to be between 0 and 1
        S_HAM_3 = np.maximum(cp['3_HAM_GLU_SG']*(S_GLU_3_interim), 0)
    ...
    def set_control_params(self, params):
        if len(params) != self.n_par:
            ...
            self.memCP = copy.deepcopy(self.cp) # Hold an initial copy of the set control paramters
            self.modCP = copy.deepcopy(self.cp) # Hold an copy of the modulated control paramters 
    ...
    def set_control_params_leg(self, s_leg, params):
        ...
        cp['alpha_delta'] = params[cp_map['alpha_delta']] *10*np.pi/180 +5*np.pi/180 # *10*np.pi/180 +5*np.pi/180
    ...
    def modulate_reflex_gain(self, s_leg, params):
        if len(params) != self.n_par:
            print(f"n_par: {self.n_par}, Length of cp: {len(self.mod_keys)}")
            raise Exception('error in the number of params!!')
        else:
            # self.cp[s_leg]
            # Scaling gain for each modulation parameter
            cp_map = self.cp_map
            
            # self.cp[s_leg]['theta_tgt'] = self.cp[s_leg]['theta_tgt'] + (params[cp_map['theta_tgt']]*5*np.pi/180)

            # self.cp[s_leg]['alpha_0'] = self.cp[s_leg]['alpha_0'] + (params[cp_map['alpha_0']]*5*np.pi/180)
            # self.cp[s_leg]['C_d'] = self.cp[s_leg]['C_d'] + (params[cp_map['C_d']]*2*np.pi/180)
            # self.cp[s_leg]['C_v'] = self.cp[s_leg]['C_v'] + (params[cp_map['C_v']]*2*np.pi/180)

            # self.cp[s_leg]['Tr_St_sup'] = np.clip(self.cp[s_leg]['Tr_St_sup'] + params[cp_map['Tr_St_sup']], 0, np.inf)
            # self.cp[s_leg]['Tr_Sw'] = np.clip(self.cp[s_leg]['Tr_Sw'] + params[cp_map['Tr_Sw']], 0, np.inf)

            # self.cp[s_leg]['alpha_delta'] = self.cp[s_leg]['alpha_delta'] + (params[cp_map['alpha_delta']]*5*np.pi/180)
            # self.cp[s_leg]['knee_sw_tgt'] = self.cp[s_leg]['knee_sw_tgt'] + (params[cp_map['knee_sw_tgt']]*5*np.pi/180)
            # self.cp[s_leg]['knee_tgt'] = self.cp[s_leg]['knee_tgt'] + (params[cp_map['knee_tgt']]*5*np.pi/180)
            # self.cp[s_leg]['knee_off_st'] = self.cp[s_leg]['knee_off_st'] + (params[cp_map['knee_off_st']]*5*np.pi/180)
            # self.cp[s_leg]['ankle_tgt'] = self.cp[s_leg]['ankle_tgt'] + (params[cp_map['ankle_tgt']]*5*np.pi/180)

            # for mod_noclip_idx in np.arange(len(self.mod_keys_noclip)):
            #     self.cp[s_leg][self.mod_keys_noclip[mod_noclip_idx]] = self.cp[s_leg][self.mod_keys_noclip[mod_noclip_idx]] + params[cp_map[self.mod_keys_noclip[mod_noclip_idx]]] * self.mod_keys_gains_noclip[mod_noclip_idx]

            # for mod_idx in np.arange(len(self.mod_keys_clip)):
            #     self.cp[s_leg][self.mod_keys_clip[mod_idx]] = np.clip(self.cp[s_leg][self.mod_keys_clip[mod_idx]] + (params[cp_map[self.mod_keys_clip[mod_idx]]] * self.mod_keys_gains_clip[mod_idx]) , 0, np.inf)

            for mod_idx in np.arange(len(self.mod_keys)):
                #print(f"CP: {self.cp[s_leg][self.mod_keys_Target_noclip[mod_noclip_idx]]}")
                if self.mod_keys[mod_idx] in self.mod_keys_clip:
                    self.cp[s_leg][self.mod_keys[mod_idx]] = np.clip( self.cp[s_leg][self.mod_keys[mod_idx]] + \
                        params[mod_idx] * self.mod_keys_gains[mod_idx], 0, np.inf)
                else:
                    self.cp[s_leg][self.mod_keys[mod_idx]] = self.cp[s_leg][self.mod_keys[mod_idx]] + \
                        params[mod_idx] * self.mod_keys_gains[mod_idx]

            if self.control_dimension == 3:
                
                self.cp[s_leg]['alpha_0_f'] = self.cp[s_leg]['alpha_0_f'] + (params[cp_map['alpha_0_f']]*5*np.pi/180)
                self.cp[s_leg]['C_d_f'] = self.cp[s_leg]['C_d_f'] + (params[cp_map['C_d_f']]*2*np.pi/180)
                self.cp[s_leg]['C_v_f'] = self.cp[s_leg]['C_v_f'] + (params[cp_map['C_v_f']]*2*np.pi/180)

                for ref_gain in ['1_HAB_FG', '3_HAB_Th', '3_HAB_d_Th','3_HAD_Th','3_HAD_d_Th', 
                                 '4_C_HAB_HAB_PG','4_C_HAD_HAD_PG','6_HAB_PG', '6_HAD_PG']:
                    self.cp[s_leg][ref_gain] = np.clip(self.cp[s_leg][ref_gain] + params[cp_map[ref_gain]], 0, np.inf)

            self.modCP[s_leg] = copy.deepcopy(self.cp[s_leg])

    def modulate_reflex_targets(self, s_leg, params):
        if len(params) != len(self.mod_keys_Target):
            raise Exception(f"error in the number of params!!, Should be {len(self.mod_keys_Target)}")
        else:

            for mod_idx in np.arange(len(self.mod_keys_Target)):
                #print(f"CP: {self.cp[s_leg][self.mod_keys_Target_noclip[mod_noclip_idx]]}")
                if self.mod_keys_Target[mod_idx] in self.mod_keys_Target_clip:
                    self.cp[s_leg][self.mod_keys_Target[mod_idx]] = np.clip( self.cp[s_leg][self.mod_keys_Target[mod_idx]] + \
                        params[mod_idx] * self.mod_keys_Target_gains[mod_idx], 0, np.inf)
                else:
                    self.cp[s_leg][self.mod_keys_Target[mod_idx]] = self.cp[s_leg][self.mod_keys_Target[mod_idx]] + \
                        params[mod_idx] * self.mod_keys_Target_gains[mod_idx]

            self.modCP[s_leg] = copy.deepcopy(self.cp[s_leg])

    def modulate_reflex_Simplified(self, s_leg, params):
        if len(params) != len(self.mod_keys_Simple):
            raise Exception(f"error in the number of params!!, Should be {len(self.mod_keys_Simple)}")
        else:
            #print(f"Len of param: {len(self.mod_keys_Simple)}")
            for mod_idx in np.arange(len(self.mod_keys_Simple)):
                #print(f"CP: {self.cp[s_leg][self.mod_keys_Target_noclip[mod_noclip_idx]]}")
                if self.mod_keys_Simple[mod_idx] in self.mod_keys_Simple_clip:
                    self.cp[s_leg][self.mod_keys_Simple[mod_idx]] = np.clip( self.cp[s_leg][self.mod_keys_Simple[mod_idx]] + \
                        params[mod_idx] * self.mod_keys_Simple_gains[mod_idx], 0, np.inf)
                else:
                    self.cp[s_leg][self.mod_keys_Simple[mod_idx]] = self.cp[s_leg][self.mod_keys_Simple[mod_idx]] + \
                        params[mod_idx] * self.mod_keys_Simple_gains[mod_idx]

            self.modCP[s_leg] = copy.deepcopy(self.cp[s_leg])

    def get_modulated_gains(self):
        """
        Convenience function to return gains that are modulated. (+ or - a delta value from the RL agent)
        Used for the experiment case where the reflex gains "reset" after every modulation
        Hypothesis of static reflex gains: Brain commands only suppress/enhance reflex circuits for a given timestep
        Usage: Only call this function after calling update
        """
        r_control_param = np.array([r_cp for r_cp in copy.deepcopy(self.modCP['r_leg']).values()])
        l_control_param = np.array([l_cp for l_cp in copy.deepcopy(self.modCP['l_leg']).values()])
        combined = np.hstack((r_control_param,l_control_param))
        return combined
    
    def get_spinal_phase(self):

        spinal_dict = copy.deepcopy(self.spinal_control_phase)
        r_spinal = np.array([r_cp for r_cp in spinal_dict['r_leg'].values()])
        l_spinal = np.array([l_cp for l_cp in spinal_dict['l_leg'].values()])

        return np.hstack((r_spinal,l_spinal))
    
    def get_supraspinal_command(self):

        supraspinal_dict = copy.deepcopy(self.supraspinal_command)
        r_supspinal = np.array([r_cp for r_cp in supraspinal_dict['r_leg'].values()])
        l_supspinal = np.array([l_cp for l_cp in supraspinal_dict['l_leg'].values()])

        return np.hstack((r_supspinal,l_supspinal))